...
Register RiscV64Spiller::GenerateReadCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
    ...
    int real_offset= function->GetParaSize()+raw_stk_offset; // sp+parasize+spill_offset

    //若长度在范围内，直接ld
    if(real_offset>=-2048&&real_offset<=2047){
        if(type==INT64){
            cur_block->push_back(rvconstructor->ConstructIImm(RISCV_LD,read_mid_reg,GetPhysicalReg(RISCV_sp),real_offset));
        }else if(type==FLOAT64){
            cur_block->push_back(rvconstructor->ConstructIImm(RISCV_FLD,read_mid_reg,GetPhysicalReg(RISCV_sp),real_offset));
        }
        
    }else{//长度超出范围，先计算完整偏移，再寻址
        Register offset_reg=function->GetNewReg(INT64);
        cur_block->insert(it, rvconstructor->ConstructUImm(RISCV_LI, offset_reg, real_offset));
        Register whole_offset_reg=function->GetNewReg(INT64);
        cur_block->insert(it, rvconstructor->ConstructR(RISCV_ADD, whole_offset_reg, GetPhysicalReg(RISCV_sp), offset_reg));
        if(type==INT64){
            cur_block->push_back(rvconstructor->ConstructIImm(RISCV_LD,read_mid_reg,whole_offset_reg,0));
        }else if(type==FLOAT64){
             cur_block->push_back(rvconstructor->ConstructIImm(RISCV_FLD,read_mid_reg,whole_offset_reg,0));
        }
...
Register RiscV64Spiller::GenerateWriteCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
    ...
    if(real_offset>=-2048&&real_offset<=2047){
        if(type==INT64){
            cur_block->push_back(rvconstructor->ConstructSImm(RISCV_SD,write_mid_reg,GetPhysicalReg(RISCV_sp),real_offset));
        }else if(type==FLOAT64){
            cur_block->push_back(rvconstructor->ConstructSImm(RISCV_FSD,write_mid_reg,GetPhysicalReg(RISCV_sp),real_offset));
        }
        ...
        if(type==INT64){
            cur_block->push_back(rvconstructor->ConstructSImm(RISCV_SD,write_mid_reg,whole_offset_reg,0));
        }else if(type==FLOAT64){
            cur_block->push_back(rvconstructor->ConstructSImm(RISCV_FSD,write_mid_reg,whole_offset_reg,0));
        }
