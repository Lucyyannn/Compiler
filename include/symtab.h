...
class IdTable {
private:
    std::map<std::string, Symbol> id_table{}; //string-->Symbol（由字符串找到对应的结构体指针）
                                              // Symbol->get_string()由Symbol指针找到相应字符串
...
private:
    ...
    //symbol_table是一个向量，第i位存储scope==i作用域内的symbols
...
/* 新增函数 ： */
    //类似lookup_val,但是返回对象的引用，便于修改其值
    //调用前提：Symbol C确定存在于symbol_table
    VarAttribute& get_val(const Symbol C);
...
private:
    ...
    std::vector<std::map<Symbol,VarAttribute>>ArrayAttribute;
...
public:
    ...
    VarAttribute lookup_val(Symbol C);
    void add_Array_Val(Symbol C,VarAttribute& val);
};
