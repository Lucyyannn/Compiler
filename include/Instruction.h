...
class BasicOperand {
public:
    enum operand_type { REG = 1, IMMI32 = 2, IMMF32 = 3, GLOBAL = 4, LABEL = 5, IMMI64 = 6 , IMMI8= 7, IMMI1 = 8};
...
//新增I8 I1
// @integer8 immediate
class ImmI8Operand : public BasicOperand {
    uint8_t immVal;

public:
    int GetIntImmVal() { return immVal; }

    ImmI8Operand(uint8_t immVal) {
        this->operandType = IMMI8;
        this->immVal = immVal;
    }
    virtual std::string GetFullName();
};
// @integer1 immediate
class ImmI1Operand : public BasicOperand {
    bool immVal;

public:
    int GetIntImmVal() { return immVal; }

    ImmI1Operand(bool immVal) {
        this->operandType = IMMI1;
        this->immVal = immVal;
    }
    virtual std::string GetFullName();
};
...
public:
    ...
    // @Operand datatypes
    enum LLVMType { I32 = 1, FLOAT32 = 2, PTR = 3, VOID = 4, I8 = 5, I1 = 6, I64 = 7, DOUBLE = 8 ,FPTR=9};
...
public:
    ...
    LLVMType GetDataType(){return type;}
...
public:
    ...
    LLVMType GetReturnType(){return ret_type;}
    Operand GetResult(){return result;}
...
如果你不需要返回值，将ret_val设置为nullptr即可
*/
...
public:
    ...
    int maxReg = -1; // 新增，记录每个函数暂时的最大reg数，便于后面phi插入添加变量
    ...
    std::vector<enum LLVMType> GetFormalTypes(){return formals;}
