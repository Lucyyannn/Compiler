...
public:
    ...
    std::vector<int> dimsDeclined{};
    bool is_left;
    Operand op_ptr;
    // 如果dims为nullptr, 表示该变量不含数组下标, 你也可以通过其他方式判断，但需要修改SysY_parser.y已有的代码

    int scope = -1;    // 在语义分析阶段填入正确的作用域
    Lval(Symbol n, std::vector<Expression> *d) : name(n), dims(d), is_left(false), op_ptr(nullptr) {}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};
...
public:
    ...
    ConstInitVal_exp(Expression e) : exp(e){isLval=0;}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};
...
public:
    ...
    bool isLval;
    bool is_exp(){return true;}
    VarInitVal_exp(Expression e) : exp(e) {isLval=0;}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};
...
public:
    int scope = -1;    // 在语义分析阶段填入正确的作用域
    bool has_initval=0;
    Type::ty type_decl; // 新增定义数据类型
    virtual Symbol get_name()=0;
    virtual InitVal& get_init()=0;
    virtual bool DimsExist()=0;
    virtual std::vector<Expression>& get_dims()=0;//该函数必须在dims非空的前提下调用
};
typedef __Def *Def;
...
public:
    ...
    std::vector<Expression> *dims;
    bool DimsExist(){return dims!=nullptr;}
    std::vector<Expression>& get_dims(){
        if(dims==nullptr){
            throw std::runtime_error("Cannot return a quote of dims when it is nullptr"); 
        }
        return *dims;
    }
    InitVal& get_init(){
         throw std::runtime_error("VarDef_no_init does not have an InitVal.");
    }
    // 如果dims为nullptr, 表示该变量不含数组下标, 你也可以通过其他方式判断，但需要修改SysY_parser.y已有的代码
    VarDef_no_init(Symbol n, std::vector<Expression> *d) : name(n), dims(d) {has_initval=0;}
...
public:
    ...
    VarDef(Symbol n, std::vector<Expression> *d, InitVal i) : name(n), dims(d), init(i) {has_initval=1;}
...
public:
    ...
    InitVal init;
    InitVal& get_init(){return init;}
    ConstDef(Symbol n, std::vector<Expression> *d, InitVal i) : name(n), dims(d), init(i) {has_initval=1;}
...
public:
    bool const_tag=0;
    virtual Type::ty get_type()=0;
    virtual std::vector<Def>& get_def()=0;
};
...
public:
    ...
    Type::ty get_type(){return type_decl;}
    std::vector<Def>& get_def(){return *var_def_list;}
    // construction
    VarDecl(Type::ty t, std::vector<Def> *v) : type_decl(t), var_def_list(v) {const_tag=0;}
...
public:
    ...
    std::vector<Def>& get_def(){return *var_def_list;}
    // construction
    ConstDecl(Type::ty t, std::vector<Def> *v) : type_decl(t), var_def_list(v) {const_tag=1;}
...
public:
    ...
    FuncRParams(std::vector<Expression> *p) : params(p), args(nullptr) {}
    void codeIR();
    void TypeCheck();
    void printAST(std::ostream &s, int pad);
};
